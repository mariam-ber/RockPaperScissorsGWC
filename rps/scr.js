...
function rockPaperScissors(){ //girls will code this function during the meeting
    ...
    const randomIndex = Math.floor(Math.random() *3); //math.floor removes decimal
        robotChoice = choices[randomIndex];
    if (userSelection === robotChoice) {
        showTie();
    } else if (
        (userSelection === "rock" && robotChoice === "scissors") || //triple equal sees if theyre same content and type (integer,string,etc)
